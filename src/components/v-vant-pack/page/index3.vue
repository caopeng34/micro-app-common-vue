<template>
  <div class="v-vant-pack-page">
    ...
    <div class="v-vant-pack-page-3_1">
      <van-tabs v-model="tabactive" v-if="tabshow" color="#1989fa" title-active-color="#1989fa" title-inactive-color="#999999">
        <van-tab v-for="(item) in tablist" :key="item.key" :name="item.key" :title="item.value"></van-tab>
      </van-tabs>
    </div>
    <div :id="pageareaid" class="v-vant-pack-page-2 v-vant-pack-page-2_1"
...
export default {
  ...
  watch: {
    tabidx(nv) {
      console.log('tabidx',nv)
      this.tabactive = nv
    },
    tabactive(nv) {
      console.log('tabactive',nv)
      this.$emit("update:tabidx", nv);
    },
    datas(nv) {
      this.datalist = nv
    },
  ...
  mounted() {
    if (this.openqueryflag) this.gettabs()
  },
  methods: {
    // 返回
    onClickLeft() {
    ...
    gettabs() {
      // 接口查询
      this.loading = true
      this.calltabapi().then(res => {
        this.loading = false
        if (res.code === 0) {
          this.tabshow = false
          this.tablist = []
          res.data.forEach(item => {this.tablist.push(item)})
          if (this.tablist.length === 0) {
            return false
          }
          this.tabidx ? (this.tabactive = this.tabidx) : (this.tabactive = this.tablist[0].key)
          setTimeout(() => {
            this.tabshow = true
          })
          this.onSearch()
